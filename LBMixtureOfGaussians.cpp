...
void LBMixtureOfGaussians::loadConfig()
{
  ...
  sensitivity = cvReadIntByName(fs, 0, "sensitivity", sensitivity);
  bgThreshold = cvReadIntByName(fs, 0, "bgThreshold", bgThreshold);
  learningRate = cvReadIntByName(fs, 0, "learningRate", learningRate);
  noiseVariance = cvReadIntByName(fs, 0, "noiseVariance", noiseVariance);
  
  showOutput = cvReadIntByName(fs, 0, "showOutput", showOutput);
