...
void VW_Marker::saveMarkedImage(IplImage* image) {
	...
	if (diretoryExists) {
		...
		IplImage **rois = extractROIRects(currFrame_, captureRects_, size);
		for (int i=0; i<size; i++)
		{
			sprintf(filePath, "%s\\%010d_%05d.jpg", directoryName.c_str(), frameNum, i);
			cvSaveImage(filePath, rois[i]);
			cvReleaseImage(&rois[i]);
		}
		delete[] rois;
	}
}
...
IplImage** VW_Marker::extractROIRects(IplImage *image, vector<CaptureRect> rects, int &size) {
	IplImage** ret;
	int s = rects.size();
	if (size < s) s = size;
	else size = s;
	...
	for (int i = 0; i<s; i++) {
		...
		ret[i] = cvCreateImage(cvGetSize(image), image->depth, image->nChannels);
